...
struct SegmentTree{
    ...
    void push_up(int node){
        ...
        tr[node].min_c = 0;
        if(tr[node << 1].min == tr[node].min) tr[node].min_c += tr[node << 1].min_c;
        if(tr[node << 1 | 1].min == tr[node].min) tr[node].min_c += tr[node << 1 | 1].min_c;
    }
    ...
    int count(int node, int L, int R, int l, int r){
        ...
        push_down(node);
        int mid = (L + R) >> 1, res = 0;
        if(l <= mid) res += count(node << 1, L, mid, l, r);
        if(r > mid) res += count(node << 1 | 1, mid + 1, R, l, r);
        return res;
    }
}mt;
...
int main(){
    ...
    for(int i = 1; i <= int(1e6); ++i) pos[i].push_back(n + 1);
