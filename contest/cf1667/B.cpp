...
template <typename Tp> void read(Tp &res){
	static char ch; ch = getchar(), res = 0; bool flag = false;
...
int main(){
	...
	while(T--){
		...
		for(int i = 1; i <= n; ++i) read(s[i]), s[i] += s[i - 1], t.push_back(s[i]);
		std::sort(t.begin(), t.end()), t.resize(std::unique(t.begin(), t.end()) - t.begin());
		for(int i = 0; i <= n; ++i)
			x[i] = std::lower_bound(t.begin(), t.end(), s[i]) - t.begin() + 1;
		m1.clear(), m2.clear(), m3.clear();
		m1.update(x[0], 0), m2.update(x[0], 0), m3.update(x[0], 0);
		for(int i = 1; i <= n; ++i){
			dp[i] = std::max({m1.query(x[i] - 1) + i, m2.query(x[i]), m3.query(n + 1) - i});
