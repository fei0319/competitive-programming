...
struct Matrix{
	...
	Matrix operator*(const Matrix &b)const{
		...
		for(int i = 0; i < 2; ++i)
			for(int j = 0; j < 2; ++j)
				for(int k = 0; k < 2; ++k)
					res.a[i][j] = (res.a[i][j] + (ull)a[i][k] * b.a[k][j]) % mod;
		return res;
	}
	void out(){
		for(int i = 0; i < 2; ++i){
			for(int j = 0; j < 2; ++j) printf("%u ", a[i][j]);
			puts("");
		}
	}
...
int main() {
	...
	init.a[0][0] = 1, init.a[0][1] = 2ull * qpow((ull)n * n % mod, mod - 2) % mod;
	
	tran.a[0][0] = ((ull)(n - 1) * (n - 1) - 1u) % mod * qpow((ull)n * n % mod, mod - 2) % mod;
