...
public:
	...
	static void modify(int node, int L, int R, int x, const int &val) {
		...
		int mid = (L + R) / 2;
		if (x <= mid) modify(tr[node].ls = copy(tr[node].ls), L, mid, x, val);
...
struct std::hash<std::pair<T1, T2>> {
	std::size_t operator()(const std::pair<T1, T2> &b) const {
		return std::hash<T1>()(b.first) ^ (std::hash<T2>()(b.second) + std::hash<T1>()(b.first));
	}
};
...
void solve(void) {
	...
	for (int i = 1; i <= n; ++i) {
		if (p[i] == i) {
			...
			if (t == i - 1) {
				auto [l, r] = std::pair<int, int>{ps::find(pre[i], 1, n, i + 1, n), ps::find(suf[i], 1, n, 1, i - 1)};
				mt[{pos[l], pos[r]}] += 1;
			} else if (t == i) {
				tot += 1;
