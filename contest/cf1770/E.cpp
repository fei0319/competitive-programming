...
void solve(void){
	...
	for(int i = 1; i < n; ++i){
		...
		ans = (ans +
		(ll)p1 * calc(sz[v[i]] + 1) + (ll)p2 * calc(sz[v[i]] - 1) + (ll)p3 * calc(sz[v[i]]))
		% mod;
		
		int pu = p[u[i]], pv = p[v[i]];
		p[u[i]] = ((ll)pu * (pv + ll(1 - pv) * half % mod) + ll(1 - pu) * pv % mod * half % mod) % mod;
		p[v[i]] = ((ll)pv * (pu + ll(1 - pu) * half % mod) + ll(1 - pv) * pu % mod * half % mod) % mod;
	}
