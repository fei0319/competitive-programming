...
int dp[maxn][3], fact[maxn], ifact[maxn];
...
int sov(int n){
	...
	for(int i = 4; i <= n; i += 4) H[i] = (ll)H[i - 4] * (i - 1) % mod * (i - 2) % mod * (i - 3) % mod * 2 % mod;
	for(int i = 0; i <= n; i += 2) f[i] = (ll)F[i] * ifact[i] % mod;
	for(int i = 0; i <= n; ++i) g[i] = ifact[i];
	fstdlib::poly res = f * g; res.resize(n + 1);
	int ans = 0;
	for(int i = 0; i * 4 <= n; ++i){
		int tmp = (ll)res[n - i * 4] * fact[n - i * 4] % mod;
		ans = (ans + (ll)tmp * binom(n - i * 2, i * 2) % mod * F[i * 2] % mod * qpow(2, i) % mod) % mod;
	}
	return ans;
}
