...
ll query(int sx, int sy, int gx, int gy){
	if(sx / b != gx /b && sy / b != gy / b) return __query(sx, sy, gx, gy);
	if(sx / b != gx / b) std::swap(sx, sy), std::swap(gx, gy);
	if(sy == gy) return delta(sx, gx);
	return std::min(
	{
	delta(sx, flr(sx)) + delta(gx, flr(gx)) + __query(flr(sx), sy, flr(gx), gy),
	delta(sx, flr(sx)) + delta(gx, cel(gx)) + __query(flr(sx), sy, cel(gx), gy),
	delta(sx, cel(sx)) + delta(gx, flr(gx)) + __query(cel(sx), sy, flr(gx), gy),
	delta(sx, cel(sx)) + delta(gx, cel(gx)) + __query(cel(sx), sy, cel(gx), gy)
	}
	);
}
...
void solve(void){
	...
	for(int i = 0; i < 4; ++i) for(int j = 0; j < 4; ++j){
		...
		chkmin(ans, res * k + query(s[i].first, s[i].second, g[j].first, g[j].second));
	}
