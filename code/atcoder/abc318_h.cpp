 ...
 * latest version at https://github.com/fei0319/poly.h If you have
 * discovered any bugs, please report about it at
 * https://github.com/fei0319/poly.h/issues/new
 */
...
namespace fstdlib {
    ...
    poly log(const poly &h) {
        if ((h[0] - 1) % mod)
            throw std::logic_error(
                _FEISTDLIB_SECURITY_::OUT_OF_CONVERGENCE_RADIUS);
...
int main() {
    ...
    poly F(n + 1);
    for (int i = 1; i <= n; ++i) {
        F[i] = qpow((ll)i * i % MOD, MOD - 2);
    }
    F = exp(F);

    int res = (ll)F[n] * fact[n] % MOD * fact[n] % MOD;

    int ans = (ll)fact[n] * fact[n] % MOD;
    ans = (ans - res * 2LL) % MOD;
    ans = (ans + fact[n]) % MOD;
    ans = (ans + MOD) % MOD;
    std::cout << ans << '\n';
