...
int main() {
    ...
    for (int i = 1; i < p; ++i) {
        for (int j = k + 1; j <= k + k; ++j)
            out("%d ", k * 2 * (i - 1) + j);
        for (int j = 1; j <= k; ++j) out("%d ", k * 2 * (i - 1) + j);
    }
    if (n - (p - 1) * (k + k) <= k * 3) {
        for (int i = (p - 1) * (k + k) + k + 1; i <= n; ++i) out("%d ", i);
        for (int i = (p - 1) * (k + k) + 1; i <= (p - 1) * (k + k) + k;
             ++i)
            out("%d ", i);
    } else {
        int m = n - (p - 1) * (k + k) - k * 3, s = (p - 1) * (k + k);
        for (int i = s + 1; i <= s + k; ++i) out("%d ", i + k);
        for (int i = s + k + 1; i <= s + k + m; ++i) out("%d ", i - k);
