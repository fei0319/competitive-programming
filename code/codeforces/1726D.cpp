...
// Annual Contest 2022 URL: https://codeforces.com/contest/1726/problem/D
// Memory Limit: 256 MB Time Limit: 2000 ms
//
...
void sov() {
    ...
    while (true) {
        ...
        std::vector<int> mk;
        for (int i = 1; i <= m; ++i)
            if (fa[u[i]] != v[i] && fa[v[i]] != u[i]) mk.push_back(i);
        if (m == n || m == n + 1) {
            for (int i = 1; i <= m; ++i) ans[i] = 0;
            for (int i : mk) ans[i] = 1;
            for (int i = 1; i <= m; ++i) putchar(ans[i] ? '1' : '0');
            puts("");
            return;
        }
        std::vector<int> all;
        for (int i : mk) all.push_back(u[i]), all.push_back(v[i]);
        std::sort(all.begin(), all.end());
        all.resize(std::unique(all.begin(), all.end()) - all.begin());
        if ((int)all.size() == 3) continue;
        for (int i = 1; i <= m; ++i) ans[i] = 0;
        for (int i : mk) ans[i] = 1;
        for (int i = 1; i <= m; ++i) putchar(ans[i] ? '1' : '0');
        puts("");
