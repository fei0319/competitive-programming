...
int main(){
	...
	while(m--){
		...
		for(int x : all){
			for(int i = s; i; i = s & (i - 1)) mul(tmp[i | rev(s)], dp[x][i | rev(s)]);
			mul(tmp[0 | rev(s)], dp[x][0 | rev(s)]);
		}
		for(int i = s; i; i = s & (i - 1))
			add(ans, (__builtin_popcount(s ^ i) & 1 ? -1 : 1) * tmp[i | rev(s)]);
		add(ans, (__builtin_popcount(s ^ 0) & 1 ? - 1 : 1) * tmp[0 | rev(s)]);
		printf("%d\n", (ans + mod) % mod);
	}
}