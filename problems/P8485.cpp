...
int main(){
	...
	for(int i = 1; i <= n; ++i){
		for(int l = 1, r; l <= m; l = r + 1){
			...
			if(map[i][l]){
				...
				bool ok = true;
				for(int j = l; j <= r; ++j)
					if((i > 1 && map[i - 1][j]) || (i < n && map[i + 1][j])){
						ok = false;
						break;
					}
				if(!ok) continue;
	...
	for(int i = n; i >= 1; --i){
		...
		for(int l = 1, r; l <= m; l = r + 1){
			...
			if(map[i][l]){
				...
				for(int j = l; j <= r; ++j)
					if(i + 1 <= n && map[i + 1][j]){
						if(getf(bel[(i + 1) & 1][j]) == getf(bel[i & 1][j])) continue;
						if(getf(bel[(i + 1) & 1][j]) > first){
							fa[getf(bel[i & 1][j])] = getf(bel[(i + 1) & 1][j]);
						}
						else
							fa[getf(bel[(i + 1) & 1][j])] = getf(bel[i & 1][j]);
					}
			}
		}
	...
	for(int rt : root){
		lim = 0;
		dfs(rt, 0);
		for(int i = 0; i < lim; ++i) f[i] = (ll)f[i] * (M[0] - M[i + 1] + 1) % mod;
