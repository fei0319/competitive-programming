...
bool check(int a, int b, int c, int t){
	bool ab = (ll)(x[a] - x[b]) * (y[t] - y[b]) - (ll)(x[t] - x[b]) * (y[a] - y[b]) > 0,
		 bc = (ll)(x[b] - x[c]) * (y[t] - y[c]) - (ll)(x[t] - x[c]) * (y[b] - y[c]) > 0,
		 ca = (ll)(x[c] - x[a]) * (y[t] - y[a]) - (ll)(x[t] - x[a]) * (y[c] - y[a]) > 0;
	return ab == bc && bc == ca;
}
...
void transfer(int a, int b, int c, int x, int y, int z){
	...
	if(y > z) std::swap(y, z);
	if(x > y) std::swap(x, y);
	dp[a][b][c] = (dp[a][b][c] + (ll)dp[x][y][z] * A(n - cnt[x][y][z] - 4, cnt[a][b][c] - cnt[x][y][z] - 1)) % mod;
}
