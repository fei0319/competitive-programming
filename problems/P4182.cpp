...
int main(){
	read(n), read(k);
	for(int i = 1; i <= n; ++i) read(node[i].l), read(node[i].r), arr.push_back(node[i].l);
	node[n + 1].l = 1e9 + 3, node[n + 1].r = node[n + 1].l, arr.push_back(node[n + 1].l);
	++n;
	...
	for(int i = 1; i <= n; ++i){
		int x = std::lower_bound(arr.begin(), arr.end(), node[i].l) - arr.begin() + 1;
		if(mt.query(x) == 0) tmp[++cnt] = node[i];
		mt.update(x, 1);
	}
	k -= n - cnt, n = cnt;
	for(int i = 1; i <= n; ++i) node[i] = tmp[i];
	std::sort(node + 1, node + 1 + n,
		[](const Node &a, const Node &b){
			return a.r < b.r;
		}
	...
	for(int i = 1; i <= n; ++i){
		...
		for(int j = 0; j <= std::min(i - 1, k); ++j){
			...
			if(j == i - 1) chkmax(dp[j][i], node[i].r - node[i].l);
			if(!q[i - j].empty())
...
//			printf("dp[%d][%d] = %d\n", i, j, dp[j][i]);
		...
		for(int j = 0; j <= std::min(i - 1, k); ++j){
			...
			while(!q[i - j + 1].empty() && f(i - j + 1, q[i - j + 1].back()) <= x)
				q[i - j + 1].pop_back();
			q[i - j + 1].push_back(i);
		}
	}
