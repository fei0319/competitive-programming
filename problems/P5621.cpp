...
int main(){
	...
	for(int i = 1; i <= n; ++i){
		scanf("%lld%lld%lld%lld%lld", &node[i].a, &node[i].b, &node[i].c, &node[i].d, &node[i].bonus);
		a.push_back(node[i].a), b.push_back(node[i].b), c.push_back(node[i].c), d.push_back(node[i].d);
	}
	discrimilate(a), discrimilate(b), discrimilate(c), discrimilate(d);
	for(int i = 1; i <= n; ++i){
		node[i].a = std::lower_bound(a.begin(), a.end(), node[i].a) - a.begin() + 1;
		node[i].b = std::lower_bound(b.begin(), b.end(), node[i].b) - b.begin() + 1;
		node[i].c = std::lower_bound(c.begin(), c.end(), node[i].c) - c.begin() + 1;
		node[i].d = std::lower_bound(d.begin(), d.end(), node[i].d) - d.begin() + 1;
	}
	solve1(1, n);
	ll ans = 0;
	for(int i = 1; i <= n; ++i)
