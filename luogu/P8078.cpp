...
bool insert(int b) {
    ...
    vist[b] = true;
    std::vector<int> tmp;
    while (now >= 1 && bel[now] >= b)
        tmp.emplace_back(a[now--]);
...
void del(int x) {
    if (prev[x])
        res -= pabs(pos[x] - pos[prev[x]]);
    if (next[x])
        res -= pabs(pos[next[x]] - pos[x]);
    if (prev[x] && next[x])
        res += pabs(pos[next[x]] - pos[prev[x]]);
    if (next[x])
        prev[next[x]] = prev[x];
    if (prev[x])
        next[prev[x]] = next[x];
...
int main() {
    ...
    int B = std::sqrt(n);
    for (int i = 1; i <= n; ++i)
        read(a[i]), bel[i] = (i - 1) / B, pos[a[i]] = i;
    for (int i = 1; i <= m; ++i)
        read(q[i].l), read(q[i].r), q[i].id = i;
    std::sort(q + 1, q + 1 + m, [](const Node &a, const Node &b) {
        if (bel[a.l] != bel[b.l])
    ...
    for (int i = 1; i <= m; ++i) {
        ...
        if (insert(bel[node.l])) {
            ...
            for (int j = 1; j < all.size(); ++j)
                prev[all[j]] = all[j - 1], next[all[j - 1]] = all[j],
                res += pabs(pos[all[j]] - pos[all[j - 1]]);
